...
/* CONSTANTES */
const double GRAVITY_CONST = 6.674 * 10E-11; // Constante gravedad universal
...
int main(int argc, char const *argv[])
{
    ...
    {
        objects[i].pos_x = position_dist(gen64); // Posicion x, y, z
        objects[i].pos_y = position_dist(gen64);
        objects[i].pos_z = position_dist(gen64);
        objects[i].speed_x = 0; // Velocidad x, y, z
        objects[i].speed_y = 0;
        objects[i].speed_z = 0;
        objects[i].mass = mass_dist(gen64); // Masa
        cout<<(objects[i].mass)<<"\n";
    ...
    for (int iteration = 0; iteration < num_iterations; iteration++)
    {
        ...
        for (int i = 0; i < num_objects; i++){
            for (int j = i+1; j < num_objects-i; j++){ // TODO AÑADIR OPTI num_objetos -i-1 objects[j+i+1]
                // Se resta i y 1 para evitar comprobaciones dobles
                // Comprobar colisiones
                if (i!=j){ // Colision entre objetos diferentes que no hayan sido eliminados con anterioridad
...
void vector_gravitational_force(object object_1, object object_2, double* forces){
    ...
    if (x!=0){
        forces[0] += (GRAVITY_CONST * object_1.mass * object_2.mass*(object_2.pos_x - object_1.pos_x)) / (x*x*x);
        forces[1] += (GRAVITY_CONST * object_1.mass * object_2.mass*(object_2.pos_y - object_1.pos_y)) / (x*x*x);
        forces[2] += (GRAVITY_CONST * object_1.mass * object_2.mass*(object_2.pos_z - object_1.pos_z)) / (x*x*x);
    }
}
...
void calc_gravitational(int num_objects, int i, std::vector<object> & objects, double* forces){
    for (int j=0; j<num_objects; j++){
        if (j!=i){
            //cout<<"Forces 1 "<<j<<" ax: "<<forces[0]<<" ay: "<<forces[1]<<" az: "<<forces[2]<<"\n";
            vector_gravitational_force(objects[i], objects[j], forces);
            //cout<<"Forces 2 "<<j<<" ax: "<<forces[0]<<" ay: "<<forces[1]<<" az: "<<forces[2]<<"\n";
        }
    }
}
